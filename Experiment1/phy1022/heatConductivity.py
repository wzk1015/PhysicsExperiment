...
class heatConductivity:
    ...
    PREVIEW_FILENAME = "E:\基物实验程序\PhisicsExperiment-PhysicsExperiment-master\PhysicsExperiment\heatConductivity\Preview.docx"
    #"Preview.pdf"
    #"heatConductivity.xlsx"
    DATA_SHEET_FILENAME = "E:\基物实验程序\PhisicsExperiment-PhysicsExperiment-master\PhysicsExperiment\heatConductivity\heatConductivity.xlsx"
    REPORT_TEMPLATE_FILENAME = "E:\基物实验程序\PhisicsExperiment-PhysicsExperiment-master\PhysicsExperiment\heatConductivity\heatConductivity_empty.docx"
    #"heatConductivity_empty.docx"
    REPORT_OUTPUT_FILENAME = "E:\基物实验程序\PhisicsExperiment-PhysicsExperiment-master\PhysicsExperiment\heatConductivity\heatConductivity_empty.docx"
    #"heatConductivity_out.docx"
    ...
    def input_data(self, filename):
        ws = xlrd.open_workbook(filename).sheet_by_name('heatConductivity')
        # 从excel中读取数据
        list_q = []
        pl_m = 0
        pl_t = 0
    ...
    def func(self, x, a, b,c):
        return a*numpy.sqrt(x)*(b*numpy.square(x)+c)
    
    def calc_data(self):
        x = numpy.array([0, self.data['pl_t'], 2*self.data['pl_t'], 3*self.data['pl_t'], 4*self.data['pl_t'], 5*self.data['pl_t'], 6*self.data['pl_t'], 7*self.data['pl_t'], 8*self.data['pl_t'], 9*self.data['pl_t']])
        y = numpy.array([self.data['list_v_1']])
        xx = numpy.linspace(0, 9*self.data['pl_t'], 100)
